...
using namespace std;
// QUESTİON 1
string passGen(string words[], int numberOfWords) {
    ...
    int leftside = 0;
    int rightside = password.length() - 1;
    while (leftside < rightside) {
        char temp = password[leftside];
        password[leftside] = password[rightside];
        password[rightside] = temp;
        leftside++;
        rightside--;
    }
    return password;
...
//QUESTİON 3
bool search_letter(string sourceString,char secondValue) {
    for (int i = 0; i < sourceString.length(); i++) {
        if (sourceString[i] == secondValue) {
            return true;
        }
    }
    return false;
...
int main() {
    ...
    cout << "Enter the number of words  (between 3 and 7 ): ";
    cin>>user_number;
    while (user_number < 3 || user_number > 7) {
        cout << "User_number should  be between 3 and 7." ;
        cout << "Enter the number of words (between 3 and 7): ";
        cin >> user_number;
    }
    ...
    cout << "Password after rep_with_upper: " << new_password  ;

    string final_password = swapLetter(new_password);
    cout << "Final version of the password: " << final_password  ;
    ...
    if (search_letter(final_password, first_letter)) {
        cout << "Your name's letter  is found." ;
    } else {
        cout << "Your name's letter  is not found ." ;
    }
